...
df = pd.read_table('../Datasets/SemEval2017-task4-dev.subtask-A.english.INPUT.txt', names = ['ID', 'label', 'tweet', 'NaN'])
df1 = pd.read_table('../Datasets/SemEval2017-task4-test.subtask-A.english.txt', names = ['ID', 'label', 'tweet', 'NaN'])
df['label'] = df['label'].replace('neutral', 1)
df['label'] = df['label'].replace('negative', 0)
df['label'] = df['label'].replace('positive', 2)

df1['label'] = df1['label'].replace('neutral', 1)
df1['label'] = df1['label'].replace('negative', 0)
df1['label'] = df1['label'].replace('positive', 2)
